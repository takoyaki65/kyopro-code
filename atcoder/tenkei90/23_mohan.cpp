...
// Map
int cnt[25];  // cnt[i] := state[i]配列の要素数
bool used[25][25];
int nex0[25][1 << 18];
int nex1[25][1 << 18];
int state[25][1 << 18];
// Map[sy][str] := <idx, flag> state[sy][idx] == strとなる2番目のインデックスidx
// と、状態strの状態でy = syにキングを置いても大丈夫かどうかを表すflag
...
int main() {
  ...
  for (int i = 0; i < W; i++) {
    for (int j = 0; j < cnt[i]; j++) {
      int t = state[i][j];
      int t0 = (t >> 1);             // 遷移先0(キングを置かない)
      int t1 = (t >> 1) + (1 << W);  // 遷移先1(キングを置く)
